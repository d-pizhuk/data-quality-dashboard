...
import schema_readability_data from './data/schema_readability.json';
...
// schema readability
const label_readability =
    <DoughnutChartComp
        className={"doughnut-diagram"}
        value={schema_readability_data.label_readability.value*100}
        title={"label readability"}/>

const label_enc_info = <PieChartComp
    data={[schema_readability_data.label_readability.with_enc_info, schema_readability_data.label_readability.without_enc_info]}
    labels={["Labels With Encoding Information", "Labels Without Encoding Information"]}
    title={"encoding information ratio"}
    tooltipEnabled={false}/>

const label_nonsense_info = <PieChartComp
    data={[schema_readability_data.label_readability.existing_words, schema_readability_data.label_readability.non_existing_words]}
    labels={["Existing Words", "Non-existing words"]}
    title={"word existence ratio"}
    tooltipEnabled={false}/>

const label_style_consistency_mean =
    <DoughnutChartComp
        className={"doughnut-diagram"}
        value={schema_readability_data.label_readability.style_consistency_mean*100}
        title={"style consistency mean"}/>

const cognates_ratio_barChart =
    <StackedBarChart
        data={[[schema_readability_data.label_readability.labels_with_synonym, schema_readability_data.label_readability.labels_with_hypernym],
            [schema_readability_data.label_readability.labels_without_synonym, schema_readability_data.label_readability.labels_without_hypernym]]}
        labels={["Synonyms", "Hypernyms"]}
        pointLabels={[["Labels which have Synonym in other Labels", "Labels which doesn't have Synonym in other Labels"],
            ["Labels which have Hypernym in other Labels", "Labels which have doesn't Hypernym in other Labels"]]}
        title={"Cognates Ratio"}
        tooltipEnabled={false}
        xAxisDisplay={false}/>

const label_chartGallery = <ChartGallery mainChart={label_readability} secondaryCharts={[label_enc_info, label_nonsense_info, label_style_consistency_mean, cognates_ratio_barChart]}/>

const label_hr_scores_histogram = <Histogram data={schema_readability_data.label_schema_readability_scores} name={"Distribution of SR scores for Labels"}/>

const desc_readability =
    <DoughnutChartComp
        className={"doughnut-diagram"}
        value={schema_readability_data.desc_readability.value*100}
        title={"description readability"}/>

const desc_enc_info = <PieChartComp
    data={[schema_readability_data.desc_readability.with_enc_info, schema_readability_data.desc_readability.without_enc_info]}
...
const desc_nonsense_info = <PieChartComp
    data={[schema_readability_data.desc_readability.existing_words, schema_readability_data.desc_readability.non_existing_words]}
    labels={["Existing Words", "Non-existing words"]}
    title={"word existence ratio"}
    tooltipEnabled={false}/>
...
const desc_common_words_proportion_mean =
    <DoughnutChartComp
        className={"doughnut-diagram"}
        value={schema_readability_data.desc_readability.style_consistency_mean*100}
        title={"style consistency mean"}/>

const desc_lang_confidence_mean =
    <DoughnutChartComp
        className={"doughnut-diagram"}
        value={schema_readability_data.desc_readability.lang_confidence_mean*100}
...
const desc_hr_scores_histogram = <Histogram data={schema_readability_data.desc_schema_readability_scores} name={"Distribution of SR scores for Descriptions"}/>

const label_readability_desc =
    <div className={"desc_container half"}>
        <p><b>Label Readability</b> calculation process involves several key steps and metrics designed to evaluate the quality and clarity of <b>labels</b> (important to mention,
            that <b>titles</b> in Knowledge Graph are treated as labels):</p><br/>

        <ol>
            <li><p><b>Word Existence Evaluation</b>: For each label, the individual words are checked for their existence in a lexical database (WordNet).
                ...
                A score is then calculated for each label based on the proportion of existing words to the total number of words.</p></li><br/>
            <li><p><b>Style Consistency</b>: it is evaluated based on predefined conventions that may vary depending on whether the label represents a class, instance or property. Specific criteria include:</p>
                <ul>
                    <li><p><b>Character Validity</b>: Ensures that labels contain only valid characters and do not use improper casing conventions like PascalCase or camelCase.</p></li>
                ...
                <p>The style consistency score is derived from the adherence of the label to these conventions.</p><br/>
            </li>
            <li><p><b>Cognates Absence Calculation</b>: The evaluation continues by assessing the presence or absence of synonyms and hypernyms for the words within each label. The calculations involve
                identifying whether any synonym or hypernym exists in the unique set of words derived from other labels. A score is computed based on how many words lack synonyms and hypernyms.</p></li>
        </ol>

        <p>The final readability score for each label is calculated by taking the average of the following metrics: <b>word existence score</b>, <b>style consistency score</b>,
            <b> synonym absence score</b>, and <b>hypernym absence score</b>. Additionally, an <b>encoding penalty</b> is applied based on whether the label includes encoding information.
            Labels without such information incur a penalty, which adjusts the final readability score downwards.</p>
    </div>

const label_hr_scores_hist_desc =
    <div className={"desc_container"}>
        <p>
            This histogram is designed to help you better understand the calculations and the quality of text data in the Knowledge Graph. It displays the distribution of Schema Readability(SR) scores for <b>labels</b>.
            The SR scores are quantified in the range from 0% to 100%, which is divided into 10 intervals, each spanning a width of 10. The x-axis of the histogram is divided into 10 bins, each representing a range of SR scores. The y-axis represents
            the frequency of SR scores that fall within each of these bins. For each bin, the histogram shows how many scores fall into that particular range. Overall, the histogram visually represents how SR scores are distributed across the predefined
            score ranges.
        </p>
    </div>
...
const desc_readability_desc =
    <div className={"desc_container half"}>
        ...
        <p>The final readability score for each label is calculated by taking the average of the following metrics: <b>word existence score</b>, <b>style consistency score</b>, and
            <b> language confidence</b>. Additionally, an <b>encoding penalty</b> is applied based on whether the label includes encoding information.
...
const desc_hr_scores_hist_desc =
    <div className={"desc_container"}>
        <p>
            This histogram is designed to help you better understand the calculations and the quality of text data in the Knowledge Graph. It displays the distribution of Schema Readability(SR) scores for <b>descriptions</b>.
            The SR scores are quantified in the range from 0% to 100%, which is divided into 10 intervals, each spanning a width of 10. The x-axis of the histogram is divided into 10 bins, each representing a range of SR scores. The y-axis represents
            the frequency of SR scores that fall within each of these bins. For each bin, the histogram shows how many scores fall into that particular range. Overall, the histogram visually represents how SR scores are distributed across the predefined
            score ranges.
        </p>
...
const schema_readability_detailed_info = [
    [label_readability_desc, label_chartGallery],
    [label_hr_scores_histogram, label_hr_scores_hist_desc],
...
export const detailed_info = {
    ...
    2: {
        "name" : "schema readability",
        "detailed_data": schema_readability_detailed_info
    },
}
